...
const Basic = (props: any) => {
  ...
  const active = backlog.filter(
    (data: any) =>
      ['New', 'Researching', 'Awaiting Infor'].includes(data.status) ||
      (data.status === 'Awaiting Customer' && data.dayssincelastupdate <= 3)
  );
  console.log({ active });
  const onhold = backlog.filter(
    (data: any) =>
      data.status === 'On Hold by Customer' && (!data.action_date || data.action_date < new Date())
  );
  const solutionproposed = backlog.filter(
    (data: any) => data.status === 'Solution Proposed' && data.dayssincelastupdate > 20
  );
  const awaitingcustomer = backlog.filter(
    (data: any) => data.status === 'Awaiting Customer' && data.dayssincelastupdate > 7
  );
  const researching = backlog.filter(
    (data: any) => data.status === 'Researching' && data.dayssincelastupdate > 7
  );
  const newdata = backlog.filter(
    (data: any) => data.status === 'New' && data.dayssincelastupdate > 2
  );
  const awaitinginfor = backlog.filter(
    (data: any) => data.status === 'Awaiting Infor' && data.dayssincelastupdate > 2
  );
  const aging = backlog.filter(
    (data: any) =>
      data.daysSinceCreated > 60 &&
      ['Awaiting Customer', 'Researching', 'Awaiting Infor', 'On Hold by Customer'].includes(
        data.status
      )
  );
  const agingdev = backlog.filter(
    (data: any) => data.daysSinceCreated > 90 && ['Awaiting Development'].includes(data.status)
  );
  const callbacks = backlog.filter((data: any) => data.status === 'Awaiting Infor');
  const majorimpact = backlog.filter(
    (data: any) =>
      data.severityname === 'Major impact' &&
      !['Awaiting Development', 'Solution Proposed'].includes(data.status)
  );

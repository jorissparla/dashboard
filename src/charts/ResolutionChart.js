...
const config = {
  ...
  tooltip: {
    ...
    pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' + '<td style="padding:0"><b>{point.y:.1f} days</b></td></tr>',
    footerFormat: "</table>",
    shared: true,
    useHTML: true,
  },
  ...
  return (
    <Query query={ALL_RESOLUTIONS}>
      {({ data, loading }) => {
        ...
        const onPrem = resolutions.filter((item) => item.Deployment === "On-Premise").map((item) => [].concat(item.Number.toString(), item.avgTime));

...
const PlannedCourseAdd = ({
  ...
  <Query query={QUERY_SINGLE_COURSE} variables={{ id }}>
    {({ data, loading, error }) => {
      ...
      const { course } = data;
      return (
        <Mutation mutation={ADD_PLANNEDCOURSE_MUTATION} refetchQueries={[{ query: QUERY_ALL_COURSES }]}>
          {(addPlannedCourse) => {
            return (
              <PlannedCourseFormNew
                ...
                course={course}
                onSave={async (values) => {
                  let input = _.pick(values, ["courseid", "team", "startdate", "enddate", "type", "hours", "status", "applicable", "trainer"]);
